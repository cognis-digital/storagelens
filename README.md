...
[![PyPI](https://img.shields.io/pypi/v/cognis-storagelens.svg)](https://pypi.org/project/cognis-storagelens/)
[![CI](https://github.com/cognis-digital/storagelens/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/storagelens/actions)
...
STORAGELENS exists for one job — diffs and decodes contract storage layouts across proxy upgrades to catch storage-collision and uninitialized-slot bugs. — and does it without a SaaS bill or heavyweight setup.
...
```bash
storagelens --version
storagelens scan .                      # scan the current project
storagelens scan . --format json
storagelens scan . --fail-on high       # non-zero exit for CI gates
storagelens mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **OpenZeppelin Upgrades plugin**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`storagelens` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
